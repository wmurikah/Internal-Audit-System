// Migration & Database Restructure Script v3.0 - Run once to setup/migrate database
...
// Main entry point - run this function to execute migration
...
// Utility: Rebuild indexes manually if they get out of sync
...
// Utility: Validate database structure
...
// Utility: Delete all backup sheets (run ONLY after confirming migration success)
...
// Utility: Create default admin user if none exists
function createDefaultAdmin() {
  ...
  console.log('\n  ⚠️  SAVE THIS PASSWORD - You will need to change it on first login');
}

// PBKDF2-like hash using HMAC iterations
...
// Utility: Reset password for Super Admin user
function resetSuperAdminPassword() {
  const EMAIL = 'wmurikah@gmail.com';
  const NEW_PASSWORD = 'Hass@Admin2024!';
