...
// ─────────────────────────────────────────────────────────────
// Dashboard summary atomic updates
// ─────────────────────────────────────────────────────────────

function updateDashboardSummary_WPStatus(oldStatus, newStatus) {
  try {
    var summary = firestoreGetSystemSettings('dashboard_summary');
    if (!summary) return;
    ...
    firestoreUpdateSystemSettings('dashboard_summary', {
      wp_by_status: wpByStatus,
      last_updated: new Date().toISOString()
    });
  } catch (e) { console.warn('Dashboard WP update failed:', e.message); }
}
...
function updateDashboardSummary_APStatus(oldStatus, newStatus) {
  try {
    ...
    var updates = { ap_by_status: apByStatus, last_updated: new Date().toISOString() };
    var closedStatuses = ['Implemented', 'Verified', 'Closed', 'Not Implemented', 'Rejected'];
    if (newStatus === 'Overdue') {
      updates.overdue_action_plans = (summary.overdue_action_plans || 0) + 1;
    } else if (oldStatus === 'Overdue' && closedStatuses.indexOf(newStatus) >= 0) {
      updates.overdue_action_plans = Math.max(0, (summary.overdue_action_plans || 0) - 1);
    }
    firestoreUpdateSystemSettings('dashboard_summary', updates);
  } catch (e) { console.warn('Dashboard AP update failed:', e.message); }
}

function rebuildDashboardSummary() {
  var wps = firestoreGetAll(SHEETS.WORK_PAPERS);
  var aps = firestoreGetAll(SHEETS.ACTION_PLANS);
  var wpByStatus = {};
  (wps || []).forEach(function(wp) { var s = wp.status || 'Unknown'; wpByStatus[s] = (wpByStatus[s] || 0) + 1; });
  var apByStatus = {};
  var overdueCount = 0;
  var closedStatuses = ['Implemented', 'Verified', 'Closed', 'Not Implemented', 'Rejected'];
  var today = new Date(); today.setHours(0, 0, 0, 0);
  (aps || []).forEach(function(ap) {
    var s = ap.status || 'Unknown'; apByStatus[s] = (apByStatus[s] || 0) + 1;
    if (closedStatuses.indexOf(s) === -1 && ap.due_date) {
      var due = new Date(ap.due_date); due.setHours(0, 0, 0, 0);
      if (due < today) overdueCount++;
    }
  });
  var summary = {
    total_work_papers: wps ? wps.length : 0,
    total_action_plans: aps ? aps.length : 0,
    overdue_action_plans: overdueCount,
    wp_by_status: wpByStatus, ap_by_status: apByStatus,
    last_updated: new Date().toISOString()
  };
  firestoreSetSystemSettings('dashboard_summary', summary);
  console.log('Dashboard summary rebuilt:', JSON.stringify(summary));
  return summary;
}
