...
/**
 * Write a record to Firestore (the sole data store).
 * Called by service files after building the data object.
 *
 * @param {string} sheetName - Collection key (e.g., SHEETS.WORK_PAPERS)
...
/**
 * Legacy compat stub — always returns false (no Sheet writes).
...
// ─────────────────────────────────────────────────────────────
// Legacy Sheet Backup stubs (Firestore-only mode)
// ─────────────────────────────────────────────────────────────
// Sheet backup has been removed. These stubs keep callers from
// breaking while the UI is updated.

function getSheetBackupMode() { return 'disabled'; }
function isRealtimeBackup() { return false; }
function isSheetBackupEnabled() { return false; }
function backupToSheet() {}

function runIncrementalBackup() {
  return { success: true, processed: 0, message: 'Sheet backup is disabled (Firestore-only mode)' };
}

function runFullSheetBackup() {
  return { success: true, totalDocs: 0, report: ['Sheet backup is disabled (Firestore-only mode)'] };
}
